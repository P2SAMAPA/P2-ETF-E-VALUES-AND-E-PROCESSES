...
st.set_page_config(page_title="E-Values and E-Processes", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 E-Values and E-Processes</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Emerging alternative to p-values | Optional stopping | Sequential monitoring without multiple testing penalties</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 E-Values")
...
st.sidebar.markdown(f"**E-process length:** {config.E_PROCESS_LENGTH}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'e_values_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>E-value: {etf['e_value_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized E-value"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized E-value", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by E-value Evidence (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **E-values** are nonnegative random variables with expectation ≤1 under the null.
        - **E-processes** allow optional stopping and optional continuation.
        - Natural for sequential market monitoring without multiple testing penalties.
        - Higher e-value → stronger evidence against the null (positive expected return).
        - E-values > 1 are evidence for the alternative hypothesis.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the e-values.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("E-Values and E-Processes | Optional stopping for sequential market monitoring")
