...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = e_score(
            ...
            macro_window,
            window=config.E_PROCESS_LENGTH
        )
    ...
    top_etfs = [{"ticker": t, "e_value_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "e_process_length": config.E_PROCESS_LENGTH,
    ...
    out_file = f"output/e_values_{timestamp}.json"
